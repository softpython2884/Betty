...
import { getGuildsWithMembers, joinGuild, leaveGuild } from '../actions/guilds';
import { type Guild, type User } from '@/lib/db/schema';
...
const GuildCard = ({ guild, userGuildId, onAction }: { guild: GuildWithMembers, userGuildId: string | null | undefined, onAction: () => void }) => {
        ...
        <AppShell>
            <div className="space-y-8">
                <div>
                    ...
                    <p className="text-muted-foreground mt-2">Trouvez votre place, unissez vos forces et représentez votre faction.</p>
                </div>
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {guilds.map(guild => (
                        <GuildCard key={guild.id} guild={guild} userGuildId={user?.guildId} onAction={fetchData} />
                    ))}
                </div>
            </div>
        </AppShell>
